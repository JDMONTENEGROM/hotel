        ...
        <div class="row mb-4">
            <div class="col-12">
                <div class="d-flex justify-content-between align-items-center">
                    <div>
                        <h1 class="h3 text-gradient mb-1">¡Bienvenido de nuevo, {{ auth()->user()->name }}!</h1>
                        <p class="text-muted mb-0">Esto es lo que está pasando en tu hotel hoy</p>
                    </div>
                    <div class="text-end">
                        <div class="text-muted small">{{ now()->locale('es')->isoFormat('dddd, D [de] MMMM YYYY') }}</div>
                        <div class="fw-bold">{{ now()->locale('es')->isoFormat('hh:mm A') }}</div>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <tbody>
                                    @forelse ($transactions as $transaction)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="fw-medium">
                                                    <a href="{{ route('room.show', ['room' => $transaction->room->id]) }}"
                                                       class="text-decoration-none">
                                                        Habitación {{ $transaction->room->number }}
                                                    </a>
                                                </div>
                                            ...
                                            <td>
                                                <div class="small">
                                                    <div><strong>Entrada:</strong> {{ Helper::dateFormat($transaction->check_in) }}</div>
                                                    <div><strong>Salida:</strong> {{ Helper::dateFormat($transaction->check_out) }}</div>
                                                </div>
                                            </td>
                                            <td>
                                                @php
                                                ...
                                                <span class="badge {{ $daysLeft <= 0 ? 'bg-danger' : ($daysLeft <= 1 ? 'bg-warning' : 'bg-success') }} badge-lh">
                                                    {{ $daysLeft == 0 ? 'Último Día' : $daysLeft . ' ' . Helper::plural('Día', $daysLeft) }}
                                                </span>
                                            </td>
                                            <td>
                                                @php
                                                ...
                                                @endphp
                                                @if($balance <= 0)
                                                    <span class="text-success fw-medium">Pagado</span>
                                                @else
                                        ...
                                        <tr>
                                            <td colspan="6" class="text-center py-5">
                                                <div class="text-muted">
                                                    ...
                                                    <p class="mb-0">No hay huéspedes registrados hoy</p>
                                                    <small>¡Tu hotel está listo para nuevas reservas!</small>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforelse
            ...
            <div class="col-lg-4 mb-4">
                <div class="card card-lh h-100">
                    <div class="card-header">
                        ...
                        <small class="text-muted">Estadísticas de huéspedes para {{ Helper::thisMonth() }}/{{ Helper::thisYear() }}</small>
                    </div>
                    <div class="card-body">
                        <div class="text-center mb-4">
                            <div class="d-flex justify-content-center align-items-center">
                                <div class="text-center">
                                    ...
                                    <small class="text-muted">Total Huéspedes Este Mes</small>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-12">
                <div class="card card-lh">
                    <div class="card-header">
                        ...
                        <small class="text-muted">Tareas comunes y accesos directos</small>
                    </div>
                    <div class="card-body">
                        <div class="row">
