...
class Helper
{
    ...
    {
        return Carbon::parse($date)->locale(config('app.locale', 'es'))->isoFormat('dddd, D MMM YYYY');
    }

    public static function dateFormat($date)
    {
        return Carbon::parse($date)->locale(config('app.locale', 'es'))->isoFormat('D MMM YYYY');
    }

    // Cambiar a formato de 12 horas con indicador AM/PM localizado
    public static function dateFormatTime($date)
    {
        return Carbon::parse($date)->locale(config('app.locale', 'es'))->isoFormat('D MMM YYYY hh:mm A');
    }

    // Formato sin año en 12 horas con AM/PM
    public static function dateFormatTimeNoYear($date)
    {
        return Carbon::parse($date)->locale(config('app.locale', 'es'))->isoFormat('D MMM, hh:mm A');
    }
